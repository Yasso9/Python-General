...
def verif_synthaxe_regle(regle):
    ...
    for caractere in regle:
        if(caractere == "["):
            nombreCrochet += 1 # Un crochet a été ouvert, on a une paire en plus
        elif(caractere == "]"):
            ...
            nombreCrochet -= 1 # Un crochet a été fermé, on a une paire en moins
...
def lecture_chaine(chaine, pos_actuel, definition, tailleRegle=0):
    ...
    if(guillemet != chaine[pos_actuel]):
        raise ErreurSynthaxe("Le guillemet qui ouvre la chaine et le guillemet qui la ferme ne sont pas les mêmes")
...
def lecture_nombres(definition, chaine, pos_actuel, tailleListe, valeurNegatif=False):
    ...
    while(len(nombres) < tailleListe):
        ...
        nombre, pos_actuel = lecture_nombre(definition, chaine, pos_actuel+1, valeurNegatif)
        nombres += (nombre,) # Methode pour ajouté un nombre a un tuple
